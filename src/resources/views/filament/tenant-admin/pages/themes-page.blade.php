<x-filament-panels::page>
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-bold text-gray-900 dark:text-white">Store Themes</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Choose a theme for your store or customize one of the available themes</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            @foreach($this->getThemes() as $theme)
                ...
                <div @class([
                    ...
                    <div class="h-32 relative overflow-hidden" style="background-color: {{ $palette['background'] }}">
                        {{-- Header strip --}}
                        <div class="h-9 flex items-center px-3 gap-2"
                             style="background-color: {{ $palette['header'] }}; border-bottom: 1px solid {{ $palette['border'] ?? '#e5e7eb' }}">
                        ...
                        <div class="p-3 flex gap-2">
                            <div class="flex-1 space-y-2">
                                ...
                                <div class="mt-3 inline-flex h-5 w-16 rounded-lg items-center justify-center"
                            ...
                            <div class="w-20 h-16 rounded-xl"
                                 style="background-color: {{ $palette['card_bg'] }}; border: 1px solid {{ $palette['border_muted'] ?? '#f6f3f4' }}">
                    ...
                    <div class="p-4 bg-white dark:bg-gray-800 space-y-3">
                        <div>
                            ...
                            @if($theme->tenant_id === null)
                                <span class="text-xs text-gray-400">public theme</span>
                            @else
                                <span class="text-xs text-primary-500">your custom theme</span>
                            @endif
                        </div>
            ...
            <div wire:click="mountAction('createTheme')"
                ...
                <div class="text-center">
                    <p class="text-sm font-medium text-gray-600 dark:text-gray-400">New Theme</p>
                    <p class="text-xs text-gray-400 dark:text-gray-500">Create a custom theme</p>
                </div>
            </div>
        </div>
    </div>
