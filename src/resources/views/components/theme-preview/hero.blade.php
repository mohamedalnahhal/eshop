...
<section class="relative overflow-hidden rounded-card shadow-glow px-8 py-14 md:px-16 md:py-20">
    <div class="pointer-events-none absolute -top-20 -inset-e-20 w-72 h-72 rounded-full bg-primary/10 blur-3xl"></div>
    <div class="pointer-events-none absolute -bottom-16 -inset-s-16 w-56 h-56 rounded-full bg-primary/10 blur-3xl"></div>
        ...
        <div class="flex-1 text-center md:text-start">
            ...
            <div class="flex flex-wrap gap-3 justify-center md:justify-start">
                @if(!empty($section['cta_primary_label']))
                    <button class="btn btn-primary hover:opacity-75 font-bold rounded-cta! transition-all! shadow-glow! hover:-translate-y-0.5">
                        {{ $section['cta_primary_label'] }}
                        @icon('arrow-r', 'w-4 h-4 rtl:rotate-180')
                    </button>
                @endif
                @if(!empty($section['cta_secondary_label']))
                    <button class="btn bg-surface-200 hover:bg-surface-300 text-theme font-bold rounded-cta! transition-all! hover:-translate-y-0.5">
