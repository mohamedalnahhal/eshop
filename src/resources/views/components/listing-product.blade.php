...
<div class="group card overflow-hidden transition-all duration-500 hover:-translate-y-2">
    <a href="{{ route('shop.product.show', ['id' => $product->id]) }}" class="block relative overflow-hidden">
        <div class="aspect-square">
            @php $ImagePath = $product->media->first()?->file_path; @endphp
            @if($ImagePath)
    ...
    <div class="p-5 flex flex-col grow">
        <h2 class="line-clamp-1">
            <a href="{{ route('shop.product.show', ['id' => $product->id]) }}" class="block text-theme-xl font-bold mb-2 text-theme hover:underline">
                {{ $product->name }}
            </a>
        </h2>
        <div class="mb-4">
        ...
        <div class="mt-auto flex flex-col gap-3">
            ...
            <p class="text-muted text-theme-sm border-b border-border pb-4 mb-1 overflow-hidden">
                {{ $product->description ? Str::limit($product->description, 60) : __('No products found') }}
            </p>
