    ...
    public function run(): void
    {
        ...
        foreach ($this->tenantData() as $entry) {
            ...
            if (! $tenant->users()->where('user_id', $owner->id)->exists()) {
                $tenant->users()->attach($owner->id, ['role' => TenantUserRole::OWNER]);
            }
            ...
            try {
                ...
                $reviewUsers = Customer::inRandomOrder()->limit(5)->get();
                        ...
                        foreach ($selectedReviews as $reviewData) {
                            ...
                            foreach ($voters as $voter) {
                                ReviewVote::updateOrCreate(
                                    ['review_id' => $review->id, 'customer_id' => $voter->id],
                                    ['is_helpful' => true]
                                );
                            }
                        }
